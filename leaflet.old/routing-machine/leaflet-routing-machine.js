...
(function() {
	...
	L.Routing.Itinerary = L.Control.extend({
		...
		onAdd: function(map) {
			...
			this._radioContainer = L.DomUtil.create('div','modeListBox');
			this._radioContainer.innerHTML = '<form>  <select id = "modeListBoxSelect" name="mode"><option value="WALK">Walk</option><option value="CAR">Car</option><option value="BICYCLE">Bicycle</option> <option value="TRANSIT%2CWALK">Transit request</option> <option value="BUSISH%2CWALK">Bus</option> <option value="TRAINISH%2CWALK">Train</option> <option value="CAR_PARK%2CWALK%2CTRANSIT">Park and ride</option> <option value="CAR%2CWALK%2CTRANSIT">Kiss and ride</option>  <option value="BICYCLE_PARK%2CWALK%2CTRANSIT">Bike and ride</option> <option value="TRANSIT%2CBICYCLE">Bicycle and transit</option> </select></form>'; 
			
			this._container.appendChild( this._radioContainer  );
		...
		_createItineraryContainer: function(r) {
			...
			for (j = 0; j < r.coordinates.length; j++) {
				for (i = 0; i < r.instructions[j].length; i++) {
					...
					this._addRowListeners(step, r.coordinates[j][instr.index]);
				}
			}
			...
			for (var j = 0; j < route.coordinates.length; j++) {
				...
				if (route.instructions[j][0] != undefined){
					if (route.instructions[j][0]["mode"] == "WALK"){
						...
						this.options.styles[2]["color"] = "red";
					}
				}
				else {
						this.options.styles[2]["color"] = "red";
...
(function() {
	...
	L.Routing.OSRM = L.Class.extend({
		options: {
			...
			//  http://router.project-osrm.org/viaroute?instructions=true&alt=true&loc=60.4176962,22.4546677&loc=60.64906198236632,22.58721253095835
			//
			timeout: 30 * 1000,
			routingOptions: {}
		},
...
//			coordinates = this._decodePolyline(response["plan"]["itineraries"][0]["legs"][0]["legGeometry"]["points"]);

/*			
			coordinates = this._decodePolyline(response.route_geometry);
			actualWaypoints = this._toWaypoints(inputWaypoints, [response.via_points[0], response.via_points[1]]);
			
			
			
			console.log(this._createName(response.route_name));
...
//			console.log(response["plan"]["itineraries"].length);
			alts = [];
			...
			var routeSum = []
			var direction, type;
			var speed, indexRatio;
			
			var totalDistance = 0;
			var totalTime = 0;
			
			speed = response["plan"]["itineraries"][itinerary]["legs"][0]["distance"] / response["plan"]["itineraries"][itinerary]["legs"][0]["duration"];
			indexRatio = response["plan"]["itineraries"][itinerary]["legs"][0]["distance"] / response["plan"]["itineraries"][itinerary]["legs"][0]["legGeometry"]["length"];

//			console.log(routeSum);

			for (leg = 0; leg < response["plan"]["itineraries"][itinerary]["legs"].length; leg++) {
				totalDistance += response["plan"]["itineraries"][itinerary]["legs"][leg]["distance"];
				coordinates.push(this._decodePolyline(response["plan"]["itineraries"][itinerary]["legs"][leg]["legGeometry"]["points"]));
...
//				console.log(response["plan"]["itineraries"]);
				...
				steps[leg].push({
							...
							distance: 0,
							time: 0,
							road: response["plan"]["itineraries"][itinerary]["legs"][leg]["from"]["name"],
							direction: "EAST",
							exit: undefined,
							index: 0,
							mode: response["plan"]["itineraries"][itinerary]["legs"][leg]["mode"]
						});
				
				
				console.log(itinerary, leg, response["plan"]["itineraries"][itinerary]["legs"][leg]["steps"].length);
				for ( var i = 0; i < response["plan"]["itineraries"][itinerary]["legs"][leg]["steps"].length; i++) {
					
						steps[leg].push({
							type: response["plan"]["itineraries"][itinerary]["legs"][leg]["steps"][i]["relativeDirection"],
							distance: response["plan"]["itineraries"][itinerary]["legs"][leg]["steps"][i]["distance"],
							time: Math.round(response["plan"]["itineraries"][itinerary]["legs"][leg]["steps"][i]["distance"] / speed),
							road: response["plan"]["itineraries"][itinerary]["legs"][leg]["steps"][i]["streetName"],
							direction: response["plan"]["itineraries"][itinerary]["legs"][leg]["steps"][i]["absoluteDirection"],
							exit: undefined,
							index: Math.floor(distSum / indexRatio),
							mode: response["plan"]["itineraries"][itinerary]["legs"][leg]["mode"]
						});
						distSum = distSum + response["plan"]["itineraries"][itinerary]["legs"][leg]["steps"][i]["distance"];
				...
				steps[leg].push({
							type: "END",
							distance: 0,
							time: response["plan"]["itineraries"][itinerary]["legs"][leg]["duration"],
							road: response["plan"]["itineraries"][itinerary]["legs"][leg]["to"]["name"],
							direction: "EAST",
							exit: undefined,
							index: Math.floor(distSum / indexRatio),
							mode: response["plan"]["itineraries"][itinerary]["legs"][leg]["mode"]
			...
			//console.log(response["plan"]["itineraries"][itinerary]);
			//console.log(steps);
...
//			console.log(response);
			...
			alts.push({
				...
				inputWaypoints: inputWaypoints,
				waypoints: actualWaypoints,
				//waypointIndices: this._clampIndices([0, response["plan"]["itineraries"][itinerary]["legs"][leg]["legGeometry"]["length"]], coordinates),
...
//			console.log("-----")
		...
		buildRouteUrl: function(waypoints, options) {
			...
			//return 'http://router.project-osrm.org/viaroute?instructions=true&alt=true&loc=60.4176962,22.4546677&loc=60.64906198236632,22.58721253095835';
			//return this.options.serviceUrl + '?' + 'fromPlace=60.44655119928639%2C22.252120971679688&toPlace=60.455017040451025%2C22.276153564453125&time=12%3A43pm&date=11-01-2015&mode=WALK&maxWalkDistance=804.672&wheelchair=false&locale=en'
				// 'http://router.project-osrm.org/viaroute?instructions=true&alt=true&loc=60.4176962,22.4546677&loc=60.64906198236632,22.58721253095835'
			
			//this.options.serviceUrl = 'http://router.project-osrm.org/viaroute'
/*			
			var locs = [],
				wp,
			    computeInstructions,
...
*/
			...
			var mode = document.getElementById('modeListBoxSelect');

			var d = new Date();

//			var datestring = (d.getMonth()+1) + "-" + d.getDate() + "-" + d.getFullYear();
			var datestring = "11-01-2015";
			var timestring = d.toLocaleTimeString().replace(/([\d]+:[\d]{2})(:[\d]{2})(.*)/, "$1$3");
			
			return this.options.serviceUrl + '?' + "fromPlace=" + waypoints[0]["latLng"]["lat"] + "%2C" + waypoints[0]["latLng"]["lng"] + "&toPlace=" + waypoints[1]["latLng"]["lat"] + "%2C" + waypoints[1]["latLng"]["lng"] + "&time=" + timestring + "&date=" + datestring + "&mode=" + mode.options[mode.selectedIndex].value;
		},
